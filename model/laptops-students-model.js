...
class StudentsLaptops {
  ...
  async update(data) {
    ...
    let SQL = `UPDATE student_laptop SET std_id = $1, laptop_id = $2 , availability_student = $3 WHERE std_lapt_id = $4 RETURNING *`;
    try {
      let { rows } = await pool.query(SQL, VALUES);
      return rows;
    } catch (err) {
  ...
  async joinLaptopStudent(){
    let SQL = `SELECT * FROM laptops LEFT OUTER JOIN student_laptop ON laptops.serial_no = student_laptop.laptop_id`;
